import { ContentLayout } from '@/components/docs/content-layout'

export default function Layout({ children }) {
  return <ContentLayout section="GETTING STARTED" page="QUICK START">{children}</ContentLayout>
}

# Quick Start

Get Delimiter monitoring your AI rate limits in under 2 minutes.

## 1. Sign in at delimiter.app

Sign in with GitHub — one click, no passwords. New accounts are created automatically on first sign-in.

## 2. Copy your project key

Your project key (starts with `dlm_`) is shown on the dashboard after sign in.

This key identifies your project — it's not a secret, but keep it private.

## 3. Install the SDK

```bash
npm install @delimiter/sdk
```

## 4. Initialize Delimiter

```typescript
import { delimiter } from '@delimiter/sdk'

delimiter.init('dlm_your_project_key')
```

Add these two lines at the root of your application (e.g. `index.ts`, `server.ts`, or your app's entry point). That's it — no wrapping, no per-provider setup.

Every AI API call your app makes is now automatically monitored.

## 5. Dashboard lights up

The moment your first API call completes, your dashboard shows real-time rate limit data. Health cards appear for each provider, showing your current usage against the provider's limits.

Add a new provider later? Switch from OpenAI to Anthropic? Use LangChain or Vercel AI SDK? Delimiter detects it automatically — no code changes needed.

## 6. (Optional) Connect providers for credit monitoring

For credit balance monitoring, connect your provider accounts in the dashboard under **Connections**. Delimiter never stores your credentials — authentication is handled securely via Pipedream Connect. You authenticate directly with each provider, and Delimiter receives read-only billing data.
